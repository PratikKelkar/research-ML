...
def get_feature_matrix(category1, category2):
    ...
    tEx = 20 #number of examples to downsample to
    ...
    testingWordNumLabels = np.zeros( ((10-training_amt)*total_words))
    for i in word_list:
        wordptr+=1

        excl = [-1]*10 #the i'th index to be excluded for testing
        ...
        for bandnum in range(5):
            ...
            for pres in range(10):
                ...
                testingWordNumLabels[wordptr] = i
                ptr2+=1
    ...
    for pres in range((10-training_amt)*total_words):
        ...
        testingWordNumLabels[pres] = word_list[(pres%total_words)]

        #print("<< " + str(testingWordNumLabels[pres]) + " " + str(categors[int(testingWordNumLabels[pres])]))
    ...
    predictions = classifier.predict(grandmatrixtest)
    for iterox in range(len(predictions)):
        by_word[int(testingWordNumLabels[iterox])].append(simplepair[int(predictions[iterox])])
        print("Predicted " + str(testingWordNumLabels[iterox]) + " " + str(predictions[iterox]) + " " + str(testlabels[iterox]))
...
f = open("results_11.14.txt", "w")
g = open("categ_results.txt","w")
by_word = []
for i in range(63):
    lsto = []
    by_word.append(lsto)
...
for i in range(63):
    thingers = ""
    for j in range(len(by_word[i])):
        thingers = thingers + str(by_word[i][j]) + " "

    print(str(i) + ": " + thingers)
    g.write(str(i) + ": " + thingers + "\n")

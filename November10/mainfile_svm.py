    ...
    for band_num in range(5):
        ...
        for t in range(tStart, tEnd-tWidth+1, tIncr): #various starts of time slice
            ...
            for channel in range(256):
                featureVectorShTrain= np.zeros((training_amt*total_words,3*tEx))
                presnum = 0
                for pres in range(training_amt):
                    for wordnum in range(total_words): #pres then word ordering important to make sure ShTrain ends with even in each chunk of 10 (i.e. 1,2,3...10, 1,2,3...10, etc)
                        ...
                        for tEStart in range(t,t+tWidth-tEx+1,tEx):
                            ...
                            abs_max = 0
                            abs_min = 1000000000
                            for idx in range(tEStart,tEStart+int(tWidth/tEx)):
                                val = TrainingData[band_num][wordnum][pres][channel][idx]
                                absval = abs(val)
                                
                                if(absval > abs(abs_max)):
                                    abs_max = val
                                if(absval < abs(abs_min)):
                                    abs_min = val
                            featureVectorShTrain[presnum][feature_vector_position] = abs_max
                            feature_vector_position+=1
                            featureVectorShTrain[presnum][feature_vector_position] = abs_min
                            feature_vector_position+=1
                        presnum+=1
                avg_svm = 0
                
                for fold in range(4):
                    valid_x = featureVectorShTrain[fold*2*total_words : (fold+1)*2*total_words]
                    valid_y = trainlabels[fold*2*total_words: (fold+1)*2*total_words]
                    tt_x = np.concatenate( (featureVectorShTrain[0:fold*2*total_words],featureVectorShTrain[(fold+1)*2*total_words:8*total_words]),axis=0)
                    tt_y = np.concatenate( (trainlabels[0:fold*2*total_words],trainlabels[(fold+1)*2*total_words:8*total_words]),axis=0)
                    scaler = StandardScaler()
                    tt_x = scaler.fit_transform(tt_x)
                    valid_x = scaler.transform(valid_x)
                    classifier = LinearSVC()
                    tt_y = np.ravel(tt_y)
                    valid_y=np.ravel(valid_y)

                    acco = classifier.score(valid_x,valid_y)

                    avg_svm+=acco/4
                
                if(len(SHheap)<400):
                    heappush(SHheap, (avg_svm, band_num, t, channel, flattened))#push this btc vector
                else:
                    heappushpop(SHheap, (avg_svm, band_num, t, channel, flattened))

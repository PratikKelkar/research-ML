...
clist = np.logspace(-10, 8,num=19 ,base=2)
#gammas = np.asarray([.0001,.001,.01,.1,1])
gammas = np.logspace(-17,0,num=18,base=2)
...
def run():
    ...
    for vox in range(2447):
        ...
        x_tst, y_tst = chooseV(vox, x_test, y_test)
        jk=StandardScaler()
        vox_x = jk.fit_transform(vox_x)
        x_tst = jk.transform(x_tst)
        clft = svm.SVC(C=optC, gamma=optG, kernel='rbf', random_seed=seed)
        clft.fit(vox_x, vox_y)
        scoret = clft.score(x_tst, y_tst)

...
for bandnum in range(1):
    ...
    for t in range(tO, (tN-tW), dT):                                    #12 start times (from 100 to 650)
        for channel in range(256):                                    #256 channels
            ...
            if(len(SVMheap) < 400):
                heappush(SVMheap,(average_accuracy_svm,bandnum,t,channel,featureVectorSVM))
            else:
                heappushpop(SVMheap,(average_accuracy_svm,bandnum,t,channel,featureVectorSVM))
            ...
            if(len(SHheap) < 400):
                heappush(SHheap, (avgpearson, bandnum,t,channel,featureVectorSh))
            else:
                heappushpop(SHheap, (avgpearson, bandnum,t,channel,featureVectorSh))
...
print("SVM results:")
for i in range(400):
    (svmacc,bandnum,t,channel,featvec) = heappop(SVMheap)
    print("Band: " + str(bandnum) + " | Time: " + str(t) + " | Channel: " + str(channel)
          + " | Accuracy: " + str(svmacc))
    svm_results.append((svmacc, featvec, bandnum, t, channel))

print("===================================")

print("Pearson results")
for i in range(400):
    (prsn,bandnum,t,channel,featvec) = heappop(SHheap)
    print("Band: " + str(bandnum) + " | Time: " + str(t) + " | Channel: " + str(channel) +
          " | Pearson: " + str(prsn))
    sh_results.append((prsn, featvec, bandnum, t, channel))

...
def aug21(totalwords, cat1, cat2):
    ...
    for bandnum in range(5):
        ...
        for t in range(tO, (tN-tW+1), dT):                                    #12 start times (from 100 to 650)
            ...
            for channel in range(256):                                    #256 channels
                featureVectorSh = np.zeros((10*totalwords,tE))
                auxInfo = np.zeros((10*totalwords,3*tE))
                #create feature vector, Shinkareva for each channel
                featureVectorSVM = np.zeros((10*totalwords,3*tE))                  #create feature vector, SVM for each channel
                for presentation in range(10*totalwords):                                 #feature vectors for all 110 presentations, even though only first 88 are used for training/testing
                    posCounterSh = 0
                    posCounterSVM = 0                                                  
                    for tEstart in range(t, (t+tW-tE+1), tE):             #downsample data
                        requiredSegment = allchanneldata[presentation][channel][tEstart:tEstart+int(tW/tE)]
                        
                        sumo = 0
                        minavgv = 1000000000; minidx=-1
                        maxavgv=0; maxidx=-1
                        for itero in range(int(tW/tE)):
                        ...
                        #posCounterSVM +=1
                        featureVectorSh[presentation][posCounterSh]=requiredSegment[maxidx]
                        posCounterSh+=1
    ...
    '''
                ...
                sh_save_states[sh_curr_idx] = (bandnum,t,channel,featureVectorSh) #secretly slide in all 3 features instead of 1; labeled SVM but actually for Sh
